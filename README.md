...
- main body and wheel dimensions and masses; `drone.inertial_reference` can be `"body_only"` (`drone.mass`/`drone.inertia` describe the central body, wheels add on top) or `"total_vehicle"` (`drone.mass`/`drone.inertia` describe the whole vehicle and the analytic wheel contributions are subtracted when the MuJoCo body is built). Always set it: when the key is missing, `"body_only"` is assumed and both the Python side (`InertialReferenceWarning`) and `uavsim.Params` (`uavsim:Params:inertialReferenceUnset`) warn, because a whole-vehicle measurement read as `"body_only"` counts the wheel mass twice
